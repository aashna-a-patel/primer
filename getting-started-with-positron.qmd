...
```{r}
#| echo: false
knitr::include_graphics("getting-started-with-positron/images/terminal-1.png")
```
...
We often want a new Terminal with a different "shell," the program which serves as the interface between you and the computer. On a Mac, the default shell is referred to as "[Unix-like](https://en.wikipedia.org/wiki/Unix-like)" since it acts very much like the tradition UNIX command line. In Windows, the default shell is [PowerShell](https://en.wikipedia.org/wiki/PowerShell), an older technology which is not worth learning. To start a new Terminal with a specific shell, press the downward arrow next to the `+` sign. See the red arrow above.

```{r}
#| echo: false
knitr::include_graphics("getting-started-with-positron/images/terminal-2.png")
```

If you are using Windows, you should see an option for "Git Bash" (or maybe just "bash").

Wth luck, Git Bash will also be the default every time you open the Terminal on Windows. But, if that does not work, you should select it by hand. One way to make it the default choice is to use the "Select Default Profile" tool:

```{r}
#| echo: false
knitr::include_graphics("getting-started-with-positron/images/terminal-3.png")
```

Again, this is only useful for Windows users. But they should click on that tool and select the option that says "Git Bash" or "bash." This should set it as the default going forward.
...
```{r}
#| echo: false
knitr::include_graphics("getting-started-with-positron/images/tutorial-failure.png")
```
